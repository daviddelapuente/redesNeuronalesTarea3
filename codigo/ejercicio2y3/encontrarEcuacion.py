...
def fitnessesI(population,arbolReal,intervalo,competitors,fit):
    fitness=[]
    for i in range(len(competitors)):
        f=fit(population[competitors[i]],arbolReal,intervalo)
        fitness.append(f)
    return fitness
...
def fitnesses(population,arbolReal,intervalo,fit):
    fitness=[]
    for p in population:
        fitness.append(fit(p,arbolReal,intervalo))
    return fitness
...
def engineEncontrarEcuacion(fitnessFunction,arbolReal,intervalo,trys,populationLen,Ncompetitors,depth,F,T,mutability=1):
    b=False
    a=AST(F,T,0.5)
    print("buscando arbol para: "+str(arbolReal.__repr__()))
    ...
    for t in range(trys):
        ...
        while(len(newPopulation)<len(population)):
            ...
            fitness=fitnessesI(population,arbolReal,intervalo,competitors,fitnessFunction)
...
##############
#cosas posibles de hacer
#que el ast tenga el depth como un atributo
#que la funcion fitness se le pase al engine
#terminales y respuestas deben tener el mismo largo y respuestas debe ser una lsita de puros ceros
